...
A lib está disponível em inglês, português e espanhol. Por padrão ela é carregada em inglês, mas você pode alterar o idioma da lib para português ou espanhol da seguinte forma:

No index do seu projeto importe as funcões responsáveis pela tradução da lib, defina a estrutura de tradução e chame a função de tradução.

Aqui está um exemplo no StackBlitz:
...
Também é possível alterar as traduções para componentes individualmente, conforme o examplo abaixo
